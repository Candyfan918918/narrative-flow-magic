...
function TopicNotFound() {
  return (
    <div style={{ minHeight: '60vh', background: '#fdf0f5', padding: 60, textAlign: 'center', fontFamily: NEWS, fontStyle: 'italic', color: '#4a3040' }}>
    ...
    <div style={{ background: '#fdf0f5', color: '#0b080f', minHeight: '100vh', fontFamily: "'Inter',system-ui,sans-serif" }}>
      <main style={{ maxWidth: 900, margin: '0 auto', padding: '46px 22px 60px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {faq.map((f, i) => (
              <details key={i} style={{ background: '#fff', border: '.5px solid rgba(11,8,15,.08)', borderRadius: 12, padding: '14px 16px' }}>
                ...
                <div style={{ marginTop: 10, fontFamily: NEWS, fontStyle: 'italic', fontSize: 15, color: '#4a3040', lineHeight: 1.55 }}>{f.a}</div>
              </details>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginTop: 56, background: '#100c14', color: '#f7e8f0', borderRadius: 24, padding: '46px 30px', textAlign: 'center' }}>
          ...
          <Link to="/welcome" style={{ display: 'inline-block', background: 'linear-gradient(155deg,#ff7eb3,#e7548a 55%,#c1216b)', color: '#fff', textDecoration: 'none', padding: '14px 26px', borderRadius: 999, fontFamily: SORA, fontWeight: 700, fontSize: 14 }}>
            join shutap →
          </Link>
        </div>
      </main>
    ...
    <article style={{ background: '#fff', borderRadius: 18, padding: 20, border: '.5px solid rgba(11,8,15,.06)', boxShadow: '0 10px 28px -22px rgba(60,10,30,.28)', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <span style={{ fontFamily: NEWS, fontStyle: 'italic', fontSize: 13, color: '#4a3040' }}>{room.alias}</span>
          <span style={{ fontFamily: 'Inter', fontSize: 11, color: '#9e7a8c' }}>{room.hours}</span>
        </div>
      </header>
      <h3 style={{ fontFamily: SORA, fontWeight: 700, fontSize: 16, letterSpacing: '-.01em', lineHeight: 1.35, margin: 0, color: '#0b080f' }}>
        {room.title}
      </h3>
      <p style={{ fontFamily: NEWS, fontStyle: 'italic', fontSize: 14.5, color: '#4a3040', lineHeight: 1.55, margin: 0 }}>{snippet}</p>
      {topComment && (
        <div style={{ borderLeft: '2px solid #e7548a', paddingLeft: 12, fontFamily: NEWS, fontStyle: 'italic', fontSize: 13, color: '#6b4a5c' }}>
      ...
      <footer style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontFamily: SORA, fontWeight: 600, fontSize: 12, color: '#6b4a5c' }}>
